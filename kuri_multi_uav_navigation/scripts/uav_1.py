...
class SetpointPosition:
    ...
    def __init__(self):
        ...
        self.mapsub = rospy.Subscriber("map", Pose, self.updateposition, queue_size=1)
    ...
    def updateposition(self,p):
		...
		self.setPose(p.position.x,p.position.y,p.position.z,0,False)
		rospy.loginfo("%s, %s to %s, %s", self.currentPoseX ,self.currentPoseY,p.position.x,p.position.y)
		now = rospy.get_rostime()
		rospy.loginfo("Current time %i %i", now.secs, now.nsecs)
    ...
    def reached(self, topic):
        ...
        #rospy.loginfo("reached? %s,%s", topic.pose.position.x, topic.pose.position.y)
